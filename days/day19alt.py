...
def ways_to_make_pattern(pattern,availables,longest_avail):
    ways_first_n = Counter()
    ways_first_n[0] += 1
    for i in range(len(pattern)):
        if ways_first_n[i] == 0:
            continue
        for j in range(1,longest_avail+1):
            if pattern[i:i+j] in availables:
                ways_first_n[i+j] += ways_first_n[i]
    return ways_first_n[len(pattern)]

availables = set(lines[0].split(", "))
longest_avail = max(map(len,availables))
...
for desired in desireds:
    ways = ways_to_make_pattern(desired,availables,longest_avail)
    # print(desired)
    # print(ways)
    # input()
    if ways >= 1:
